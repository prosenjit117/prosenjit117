...
// 2021-03-12: changed null to blank for the logged out user condition as desired by Mindbody
...
{ //when bootstrapData is the source
    ...
    if (!bootstrapData.bootstrap.UserInfo) {
        bootstrapData.bootstrap = {
            UserInfo: '' //changing null to blank
        }
    }
    if (!bootstrapData.bootstrap.CompanyInfo) {
        bootstrapData.bootstrap = {
            CompanyInfo: '' //changing null to blank
        }
    }
}
...
if (window.dataStore){ //when dataStore is the source
      ...
      if (window.dataStore.config.user.isLoggedIn) { //for logged in user 
            ...
            // 2021-03-12: changed null to blank for the below logged out user condition as desired by Mindbody
             visitorId   = "";
             visitorRole = "";
             accountId   = "";
             accountName = "";      
 ...
 var xhttp = new XMLHttpRequest();
    ...
    if (jsonResponse.customAttributes){
        ...
        if (Array.isArray(companyTypeCustomAttribute) && companyTypeCustomAttribute.length && companyTypeCustomAttribute[0].valueKeys[0]==="developer" && Array.isArray(sourceIdCustomAttribute)  && sourceIdCustomAttribute.length){
            ...
            sessionStorage.setItem("appdirect_account_id",""); // 2021-03-12: changed null to blank 
            sessionStorage.setItem("appdirect_account_name",""); // 2021-03-12: changed null to blank 
    ...
    if (pCompanyName === "MINDBODY") {
        ...
        sessionStorage.setItem("appdirect_visitor_id",""); // 2021-03-12: changed null to blank 
     }
...
})('5a2e4237-9713-4f46-6d20-d236aaa35365'); //this is Prosenjit's pendo tag
